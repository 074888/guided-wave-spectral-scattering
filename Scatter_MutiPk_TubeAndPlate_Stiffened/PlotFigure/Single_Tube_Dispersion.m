...
% 0.01e6:0.01e6:0.2e6
for Frequency_Input = 0.01e6:0.01e6:0.2e6
    ...
    [Ki_Fp,Ki_Bp,Ki_Fc,Ki_Bc,Vis_Fp_U,Vis_Fp_V,Vis_Fp_W,Vis_Bp_U,Vis_Bp_V,Vis_Bp_W,Vis_Bc_U,Vis_Bc_V,Vis_Bc_W,Vis_Fc_U,Vis_Fc_V,Vis_Fc_W...
     ...
     figure(10)
     plot(Frequency_Input,Ki_Fp,"*b",Color=[0 0.4470 0.7410]);hold on

        [eige,eign,r,U,T,Un,Tn,na,ns,nsh]=circumferential1(Frequency_Input,[Rr,Rr+h],8,200);
        eige_fp = eige(abs(imag(eige))<0.01);
             figure(10)
     plot(Frequency_Input,eige_fp,"or",Marker="o",MarkerSize=8,Color=[0.6350 0.0780 0.1840]);hold on
     plot(Frequency_Input,eige_fp,"or",Marker="o",MarkerSize=6,Color=[0.6350 0.0780 0.1840]);hold on
     grid on
    xlabel("频率（Mhz）");
    ylabel("传播模的波数");
