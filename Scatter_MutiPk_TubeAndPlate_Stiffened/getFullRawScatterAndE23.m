function[S11_,S12_,S21_,S22_,E_diag_Plate,E_diag_Tube,Fin_Rcatter] = getFullRawScatterAndE23(N,Frequency_Input,Mode_N,Cons,PkPhiy,MaxLayers,...
    Layer_plate,Ang_Plate,H_Plate,Cut_Plate,....
    Rr,Layer_Tube,Ang_Tube,H_Tube,Cut_Tube,Theta,Plate_Len)
%---------------*===============*  
%--------------- ===============*    
%--------------- ===============*         
%   L Tube         R Plate      *    Muti layer
%--------------- ===============*
%----------------===============*
...
[Ki_Fp_R,Ki_Bp_R,Ki_Fc_R,Ki_Bc_R,Vis_Fp_U_R,Vis_Fp_V_R,Vis_Fp_W_R,Vis_Bp_U_R,Vis_Bp_V_R,Vis_Bp_W_R,Vis_Bc_U_R,Vis_Bc_V_R,Vis_Bc_W_R,Vis_Fc_U_R,Vis_Fc_V_R,Vis_Fc_W_R...
    ...
    ,Force_Bc_xx_R,~,~,~,Force_Bc_xz_R,Force_Bc_xy_R]=Get_AcousticField_ByMutiLayer(MaxLayers,N,Cons,Frequency_Input,Layer_plate,Ang_Plate,PkPhiy,H_Plate,Cut_Plate);
%% Get SingleLayers
[Ki_Fp_L,Ki_Bp_L,Ki_Fc_L,Ki_Bc_L,Vis_Fp_U_L,Vis_Fp_V_L,Vis_Fp_W_L,Vis_Bp_U_L,Vis_Bp_V_L,Vis_Bp_W_L,Vis_Bc_U_L,Vis_Bc_V_L,Vis_Bc_W_L,Vis_Fc_U_L,Vis_Fc_V_L,Vis_Fc_W_L...
    ...
    ,Force_Bc_xx_L,~,~,~,Force_Bc_xz_L,Force_Bc_xy_L]=Get_AcousticField_ByMutiLayer_Tube(MaxLayers,N,Rr,Cons,Frequency_Input,Layer_Tube,Ang_Tube,H_Tube,Cut_Tube);  

%% GetAll Ori-Acoustic Field
%Normalizations-JAll:MutiLayers
[~,JAll_Fp_L]=Normalize_M(Line_L,N,H_Plate,Vis_Fp_U_L,Vis_Fp_V_L,Vis_Fp_W_L,Force_Fp_xx_L,Force_Fp_xy_L,Force_Fp_xz_L);
[~,JAll_Bp_L]=Normalize_M(Line_L,N,H_Plate,Vis_Bp_U_L,Vis_Bp_V_L,Vis_Bp_W_L,Force_Bp_xx_L,Force_Bp_xy_L,Force_Bp_xz_L);

[~,JAll_Fc_L]=Normalize_M(Line_L,N,H_Plate,Vis_Fc_U_L,Vis_Fc_V_L,Vis_Fc_W_L,Force_Fc_xx_L,Force_Fc_xy_L,Force_Fc_xz_L);
[~,JAll_Bc_L]=Normalize_M(Line_L,N,H_Plate,Vis_Bc_U_L,Vis_Bc_V_L,Vis_Bc_W_L,Force_Bc_xx_L,Force_Bc_xy_L,Force_Bc_xz_L);

%----------------------------------------------------------------------------------------------------------------
[~,JAll_Fp_R]=Normalize_M(Line_R,N,H_Tube,Vis_Fp_U_R,Vis_Fp_V_R,Vis_Fp_W_R,Force_Fp_xx_R,Force_Fp_xy_R,Force_Fp_xz_R);
[~,JAll_Bp_R]=Normalize_M(Line_R,N,H_Tube,Vis_Bp_U_R,Vis_Bp_V_R,Vis_Bp_W_R,Force_Bp_xx_R,Force_Bp_xy_R,Force_Bp_xz_R);

[~,JAll_Fc_R]=Normalize_M(Line_R,N,H_Tube,Vis_Fc_U_R,Vis_Fc_V_R,Vis_Fc_W_R,Force_Fc_xx_R,Force_Fc_xy_R,Force_Fc_xz_R);
[~,JAll_Bc_R]=Normalize_M(Line_R,N,H_Tube,Vis_Bc_U_R,Vis_Bc_V_R,Vis_Bc_W_R,Force_Bc_xx_R,Force_Bc_xy_R,Force_Bc_xz_R);
...
LineOfE_Plate = [ -E_Bp_R(line_Part1) ; E_Fp_R(line_Part1) ];
LineOfE_Tube = [ -E_Bp_L(line_Part1) ; E_Fp_L(line_Part1) ];

E_diag_Plate = {};
E_diag_Plate{1}=diag( exp( 1i*LineOfE_Plate(1:length(LineOfE_Plate)*0.5)*Plate_Len) );
E_diag_Plate{2}=diag( exp( 1i*LineOfE_Plate(length(LineOfE_Plate)*0.5+1:length(LineOfE_Plate))*Plate_Len) );

E_diag_Tube = {};
E_diag_Tube{1}=diag( exp( 1i*LineOfE_Tube(1:length(LineOfE_Tube)*0.5)*Theta) );
E_diag_Tube{2}=diag( exp( 1i*LineOfE_Tube(length(LineOfE_Tube)*0.5+1:length(LineOfE_Tube))*Theta) );
%% -
LineOfTube = length(Ki_Fp_L);
LineOfPlate = length(Ki_Fp_R);

Line_Tube = 1:LineOfTube;
Line_Plate = 1:LineOfPlate;
Fin_Rcatter = zeros(LineOfTube+1+LineOfPlate,LineOfTube+1+LineOfPlate);

Fin_Rcatter(Line_Tube,Line_Tube) = SCA(Line_Tube,Line_Tube);
Fin_Rcatter(Line_Tube,Line_Plate+LineOfTube+1) = SCA(Line_Tube,Line_Plate+Mode_N);

Fin_Rcatter(1+LineOfTube+Line_Plate,Line_Tube) = SCA(Mode_N+Line_Plate,Line_Tube);
Fin_Rcatter(1+LineOfTube+Line_Plate,LineOfTube+Line_Plate+1) = SCA(Mode_N+Line_Plate,Line_Plate+Mode_N);