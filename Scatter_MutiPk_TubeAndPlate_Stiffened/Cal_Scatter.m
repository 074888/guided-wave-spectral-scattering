Fullpath = fullfile(pwd,'AuxiliaryFunc');
addpath(Fullpath);
...
[Ki_Fp_M,Ki_Bp_M,Ki_Fc_M,Ki_Bc_M,Vis_Fp_U_M,Vis_Fp_V_M,Vis_Fp_W_M,Vis_Bp_U_M,Vis_Bp_V_M,Vis_Bp_W_M,Vis_Bc_U_M,Vis_Bc_V_M,Vis_Bc_W_M,Vis_Fc_U_M,Vis_Fc_V_M,Vis_Fc_W_M...
    ...
    ,Force_Bc_xx_M,Force_Bc_yy_M,Force_Bc_zz_M,Force_Bc_yz_M,Force_Bc_xz_M,Force_Bc_xy_M]=Get_AcousticField_ByMutiLayer(MaxLayers,N,Cons,Frequency_Input,Layers,Ang,PkPhiy,H,GetPart_M);
%% Get SingleLayers
[Ki_Fp_S,Ki_Bp_S,Ki_Fc_S,Ki_Bc_S,Vis_Fp_U_S,Vis_Fp_V_S,Vis_Fp_W_S,Vis_Bp_U_S,Vis_Bp_V_S,Vis_Bp_W_S,Vis_Bc_U_S,Vis_Bc_V_S,Vis_Bc_W_S,Vis_Fc_U_S,Vis_Fc_V_S,Vis_Fc_W_S...
    ...
    ,Force_Bc_xx_S,Force_Bc_yy_S,Force_Bc_zz_S,Force_Bc_yz_S,Force_Bc_xz_S,Force_Bc_xy_S]=Get_AcousticField_BySingleLayer(N,Cons,Frequency_Input,Stiff_S,Rou_S,StifPhy_S,PkPhiy,h,GetPart_S);
...
%% -------------------
if Reverse==0
    MRF=scat_Sp(1:MLine,1:MLine);
    MRT=scat_Sp(Mode_N+1:Mode_N+SLine,1:MLine);

    SRT=scat_Sp(1:MLine,Mode_N+1:Mode_N+SLine);
    SRF=scat_Sp(Mode_N+1:Mode_N+SLine,Mode_N+1:Mode_N+SLine);
    
    Fin_Scatter_TureRevers = Fin_Scatter;
else
    MRF=scat_Sp(1:MLine,1:MLine);
    MRT=scat_Sp(Mode_N+1:Mode_N+SLine,1:MLine);
    ...
    Fin_Scatter_TureRevers = [SRF zeros(SLine,1) MRT;
                              zeros(1,SLine+1+MLine);...
                              SRT zeros(MLine,1) MRF];
